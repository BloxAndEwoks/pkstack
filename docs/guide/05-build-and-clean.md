...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) runs `/deslop` on the diff before each commit and applies [`/unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. [`/deslop`](../../skills/deslop/SKILL.md) is a diff-hygiene pass over what you are about to commit: it removes narrating comments, unsupported guards, dead compatibility paths, and unrelated edits, and reports each cut in a line. It never changes behavior; anything that would is flagged for you instead.
