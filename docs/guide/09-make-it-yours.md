...
[`/close-unit`](../../skills/close-unit/SKILL.md) mints lessons into the repo's own finding ledger. It clusters the unit's findings by mechanism, then for each cluster asks the one question that matters: does a standing lesson already cover this? If yes, the statement was right and its medium failed, so the fix is to move it up a rung, not to reword it. If no, it mints the weakest lesson that still decides every member. Its process sampler can add findings about how the run itself went, from the transcript or the decisions trail.

Lessons about the plugin rather than the repo queue as exports, and nothing leaves the repo until you approve it. Approve a lesson only if it would change a future decision. One weird session is an anecdote, not a rule.
