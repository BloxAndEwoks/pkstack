...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript under this workspace's own directory in `~/.claude/projects/` (the slug is the workspace path with every "/" turned into "-", the leading one included). Don't glob across `~/.claude/projects/*/`; that reads unrelated private chats. Walk the log against what actually happened:
...
Before handing back, you must spawn a subagent on a different model family from the one that did the work. This is the driver's step at the end of a run, never spawned by a delegate. It reviews the trail, and it is not the PR's non-author verifier, which drives the product. Self-review is not a substitute; the point is fresh eyes you cannot bring yourself. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
