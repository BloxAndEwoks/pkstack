...
Transcripts live at `~/.claude/projects/<slug>/*.jsonl`, where `<slug>` is the workspace path with every "/" turned into "-", the leading one included (so `/Users/you/proj` becomes `-Users-you-proj`). Each line is one JSON message. Stay inside the active workspace's own `<slug>` directory: do not glob across `~/.claude/projects/*/`, that crosses workspace boundaries and reads private chats from unrelated projects, and never read the `memory/` subdirectory there.
...
3. Fan out across your chat history. Spawn parallel subagents on a fast, cheap model, each taking a slice of the corpus, since searching transcripts is grunt work. Tell every subagent to order candidates by real modification time (`ls -t`) and never by UUID name, read each candidate's first line to match its opening user message against the topic, grep the topic first and then read only the matching chats and only their relevant regions, and skip the current chat plus obvious noise (subagent, eval, and test chats). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the chat UUID. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.
