...
This skill orchestrates three others: an inline mining pass (see step 1), the **Authoring or modifying a skill** playbook (`../poteto-mode/playbooks/authoring-a-skill.md`), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
Locate the active workspace's transcripts before fanning out. They live in this workspace's own directory under `~/.claude/projects/` (the slug is the workspace path with every "/" turned into "-", the leading one included). Use only that path. Don't glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Author the skill through the **Authoring or modifying a skill** playbook (`../poteto-mode/playbooks/authoring-a-skill.md`). Placement:
...
- The **Authoring or modifying a skill** playbook: skill authoring process and writing guidelines.
