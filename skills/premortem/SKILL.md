...
1. **The frame.** Not "6 months from now the business failed" but "this checkpoint SHIPPED and then:
   wedged the lifecycle, lost or double-moved money, silently dropped data, disagreed with what the
   customer was told, or fell over under a concurrent/crafted request. Explain how." Ground failure
   reasons in the actual design + codebase, not market dynamics.

2. **Generate first, then sweep the repo's RECORDED classes.** Run the raw premortem exactly as the
   method prescribes — the narrative "it shipped and died, tell the story" generation is the
...
- "What does winning look like for this?" (if you know the plan and audience but not the success criteria)
