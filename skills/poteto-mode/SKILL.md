...
- Shipping UI / IDE / CLI → the matching control skill: the repo's own verification skill, `verify-<project>`, generated by `/create-verification-skill`. It covers every surface the product ships. A repo without one gets its surface driven directly — the **run** skill (`/run`) for CLIs, servers, and apps, the Chrome MCP for browser UIs — and the missing verification skill is flagged as a risk in the same breath. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
