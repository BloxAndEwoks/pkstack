...
- The unit's final review round has run and its sweep landed findings → the **close-unit** skill, before the PR opens, run over the probe's clusters and the `decisions.tsv` trail. The final round is the sweep's triage on an ungated unit and the non-author round over the unit diff on a gated one.
...
**Use `subagent_type: "poteto-agent"` for any subagent you spawn inside a playbook step** (code-writing delegates, ad-hoc helpers). `/poteto-mode` and `poteto-agent` route through the same wrapper. Every such delegate is bound by the delegate contract in `agents/poteto-agent.md`. It executes the brief you hand it and never re-enters classification, a review round, `close-unit`, or **Opening a PR**. Routed workflow skills (`how`, `why`, `interrogate`, `close-unit`, `swarm`) set their own `subagent_type` for diverse-model review; respect what the skill prescribes, don't override to `poteto-agent`.
