...
5. Verify on the matching surface. "Inconclusive" or wrong-surface is not a pass; flag it. Every lesson the design package implicated gets its own named verification lane on that same real surface. The lanes are a floor under the surface checks, never a substitute for them.
   **5a.** Sweep the unit's diff (`BASE..head`) against the repo's finding ledger via the **verify-sweep** skill, in a neutral falsify/verify register. The package's implicated-lessons map is the work breakdown. A young or empty ledger falls back to the generic falsification classes, the seam catalogue in the **adversarial-audit** skill.
