...
- `## Verification`. State how you ran each check and its rigor. The real path is the repo's `verify-<project>` drives and their outcomes. Targeted tests are the unit floor and never the substitute on a change to a shipped surface. State the outcome of each check, not only the command name. Every unit that swept carries its ledger line here, `ledger: n lessons swept · k findings · model/fact/guard a/b/c · j fixed now · m notes with named triggers`, plus the loop-health row when **close-unit** ran.
...
**Register the unit.** Every unit is registered here, at PR open. Update its probe with the BASE and the clusters so far. Record a named close trigger alongside them: a stack closes at Shipping's ceiling, a program closes at the coordinator's wave boundary, and solo work closes on the shipping verdict this PR is about to collect. The Verification section cites the sweep's result, and the loop-health row lands with the close on this branch, after the verdict. A registered unit that never closes surfaces as an overdue register at the next close.

**The verdict before the merge.** In the solo flow, with no stack and no Graphite, the driver runs the Shipping playbook's step 1 immediately after the PR opens, so every PR carries a non-author verdict before the owner merges. That verifier is independence-first. It writes its own plan and drives the real surface through `verify-<project>` at the PR head, and it reads the ledger only afterwards, as a completeness floor. Every fourth verifier runs with no ledger at all, the blind control, counted by the driver from the ledger's loop-health rows.
