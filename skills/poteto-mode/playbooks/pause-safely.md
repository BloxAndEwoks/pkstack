...
4. Write the resume note off-context. Capture intent, what you were doing, progress and what's verified, current state, next steps, key files, gotchas, the instantiated-lessons map path, the open triage clusters, and the unit's gated classification with its BASE and named close trigger. Those last three are what a pickup agent cannot cheaply re-derive and must not rebuild blind. For the compaction trigger write it to a file like `/tmp/<slug>-resume.md`, because the in-context plan won't survive summarization. If a show-me-your-work trail exists, point at it instead of duplicating it.
