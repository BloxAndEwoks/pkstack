...
1. Locate the prior trail. A local transcript under this workspace's own directory in `~/.claude/projects/` (the slug is the workspace path with every "/" turned into "-", the leading one included; do not glob across `~/.claude/projects/*/`, that crosses workspace boundaries and reads private chats from unrelated projects), a remote agent's task, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
2. Reconstruct operational state. The branch and worktree, what already landed (`git log`, `git diff` against the base), the open todos, the decisions made, the instantiated-lessons map path, the open triage clusters, and the unit's gated classification with its BASE and named close trigger. Those last three are the artifacts a pickup cannot cheaply re-derive; read them, never rebuild them blind. The prior trail is authoritative input. Resist the bias to re-derive it.
