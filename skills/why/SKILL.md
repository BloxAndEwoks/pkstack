...
Before spawning investigators, list the MCP servers available in this session: the tools named `mcp__<server>__*` in your tool list, plus any deferred ones the session reminders name. A configured server that failed to connect is unavailable, not missing.
...
- `model`: your configured why-investigators model (default the Agent tool's `opus`, falling back to `sonnet` only when Opus returns a server error)
